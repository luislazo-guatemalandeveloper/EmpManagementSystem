...
var appEIS = angular.module('appEIS', ['ngRoute', 'angular-loading-bar', 'angularUtils.directives.dirPagination', 'ngCookies']);
...
appEIS.controller('appEISController', function ($scope, $rootScope, $location, $cookies, utilityService) {
    ...
    $rootScope.$on("$routeChangeStart", function (event, next, current) {
        ...
        if ($rootScope.Auth == 'false' && $.inArray(next.$$route.originalPath, Guest) == -1) {
            ...
            utilityService.getFile("http://localhost:61158/api/Upload", $rootScope.EmpSignIn.EmployeeId).then(function (result) {
